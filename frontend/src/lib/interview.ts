...
const defaultTopics: string[] = Object.keys(dummyQuestions);

export function getDefaultTopics(): string[] {
  return defaultTopics;
}

export function getQuestionsForTopic(topic: string, difficulty: string, count: number): Question[] {
  const pool = dummyQuestions[topic] || [];
...
export async function startInterviewSession(topic: string, difficulty: string, questionCount: number): Promise<{ id: string; questions: Question[] }> {
  ...
  // Dummy fallback
  const questions = getQuestionsForTopic(topic, difficulty, questionCount);
  return {
    id: `mock-${Date.now()}`,
    questions: questions.length > 0 ? questions : dummyQuestions['Data Structures & Algorithms'].slice(0, questionCount)
  };
}

export async function submitAnswers(sessionId: string, answers: { questionId: string; answer: string }[]): Promise<InterviewResult> {
  const token = getAccessToken();
  try {
  ...
  const totalScore = Math.floor(60 + Math.random() * 35);
  const feedbacks = [
    'Your answer shows good understanding of the core concepts. Try to provide more concrete examples to strengthen your response.',
    'Solid foundational knowledge demonstrated. Consider discussing edge cases and trade-offs more explicitly.',
    'Good approach! Your answer would benefit from more structure - try using STAR format for behavioral questions.',
    'You covered the main points well. For a stronger answer, elaborate on the implementation details and consider performance implications.'
  ];
  ...
  const weaknesses = [
    ...
    'Would benefit from mentioning edge cases'
  ];
  ...
  return {
    ...
    weaknesses: weaknesses.slice(0, 1 + Math.floor(Math.random() * 2)),
    aiFeedback: `Based on your responses, you scored ${totalScore}%. ${totalScore >= 80 ? 'Excellent work! You demonstrated strong knowledge across the topics.' : totalScore >= 65 ? 'Good effort! You have a solid foundation with some areas to improve upon.' : 'You have potential but need more practice. Focus on the key concepts highlighted below.'}`,
    improvedAnswer: 'Here is a stronger version of your answer: [Sample improved answer with better structure, specific examples, and technical depth. A strong answer would start with a clear definition, provide concrete examples, discuss trade-offs, and conclude with real-world applications.]',
