import { d as derived, w as writable } from './index-641b5f1c.js';
...
function getLevelTitle(level) {
  if (level >= 50)
    return "Interview Legend";
  if (level >= 30)
    return "Ace Architect";
  if (level >= 20)
    return "Master Candidate";
  if (level >= 15)
    return "Senior Prospect";
  if (level >= 10)
    return "Interview Expert";
  if (level >= 7)
    return "Confident Candidate";
  if (level >= 5)
    return "Rising Star";
  if (level >= 3)
    return "Apprentice";
...
export { isSidebarOpen as a, isDarkMode as i, levelInfo as l, toast as t, user as u };
//# sourceMappingURL=stores2-7b62ca2c.js.map
